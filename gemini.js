 ...
 * @param {string} taskType - 'Essay' or 'Report'
 */
export async function generateTask(taskType) {
  const ai = getClient();
 ...
 * @param {string} taskContext - The task description
 */
export async function improveWriting(text, taskContext) {
  const ai = getClient();
  ...
  Output JSON: { 
    ...
    "keyChanges": [
       "Correction: [original error] -> [correction] (Explain grammar rule if needed)",
       "Upgrade: [original word] -> [advanced vocabulary]",
       "Structure: [description of improvement]"
    ] 
  }`;
 ...
 * @param {string} taskContext - The task description
 */
export async function assessWriting(text, taskContext) {
  const ai = getClient();
